...
$mysqli->query("INSERT INTO detail_commande(id_commande, id_produit, quantite, photo) VALUES('$_POST[id_commande]','$_POST[id_produit]','$_POST[quantite]','$_POST[photo]')")or die('Erreur SQL' .$mysqli->error);
...
if (isset($_POST['ajout_panier'])) { // si on a cliqué sur le bouton d'ajout au paniervar_dump($_POST);
  ...
  ajouterProduitDansPanier($produit['titre'], $produit['id_produit'], $_POST['quantite'], $produit['prix'], $_POST['taille']); // je passe en arguments : titre, id_produit, quantité du formulaire et prix
...
if (isset($_POST['valider']) && isset($_SESSION['panier']['id_produit'])) { // si on a validé le panier
    ...
    /*for($i = 0; $i < count($_SESSION['panier']['id_produit']); $i++) {
        $id_commande = $_SESSION['commande']['id_commande'][$i];
        $id_produit = $_SESSION['panier']['id_produit'][$i];
        $quantite = $_SESSION['panier']['quantite'][$i];
        $prix = $_SESSION['panier']['prix'][$i];
        $taille = $_SESSION['panier']['taille'][$i];
        $mysqli->query("INSERT INTO detail_commande (id_commande, id_produit, quantite, taille, prix) VALUES('$id_commande', '$id_produit', '$quantite',$taille, '$prix')");
    ...
    $message = 'Merci pour votre commande. Le numéro est' . $id_commande;
...
if (empty($_SESSION['panier']['id_produit'])) { // si panier vide
    ...
    echo '<tr class="info"><th>Titre</th><th>Référence</th><th>Quantité</th><th>Prix unitaire</th><th>Taille</th><th>Action</th></tr>';
