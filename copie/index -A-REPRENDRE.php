...
$donnees = $mysqli->query("SELECT id_produit, photo, categorie FROM produit ");
while($produit = $donnees->fetch_assoc()){
$contenu .= '<h4 >'. $produit['categorie'] .'</h4>';
}
  /*$produit = $donnees->fetch_assoc();
    $contenu.= '<a href="fiche_produit.php?id_produit='. $produit['id_produit'] .'"><img src="'. $produit['photo'] .'" alt="" width="90%" height="250px"></a>';*/

if(isset($_GET['q']) && ($_GET['q']) !='NULL'){
    $donnees = $mysqli->query("SELECT id_produit, photo, categorie FROM produit WHERE categorie LIKE '%q%' ");   
/*             while($produit = $donnees->fetch_assoc()) {
                //debug($produit);    
